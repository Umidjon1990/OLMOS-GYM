...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const navItems = [
    ...
    { href: "/admin/applications", icon: ClipboardList, label: "Applications", badge: pendingCount },
  ];

  const moreItems = [
    { href: "/admin/plans", icon: List, label: "Plans" },
    { href: "/admin/trainers", icon: Dumbbell, label: "Trainers" },
    { href: "/admin/gallery", icon: Image, label: "Gallery" },
    { href: "/admin/website", icon: Settings, label: "Website Settings" },
  ];
  ...
  const NavLinks = () => (
    <div className="flex flex-col space-y-1">
      ...
      <div className="h-4" />
      <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2 px-4">Management</div>
      {moreItems.map((item) => (
        <Link key={item.href} href={item.href}>
          <Button
            variant={location === item.href ? "secondary" : "ghost"}
    ...
    <div className="min-h-[100dvh] flex flex-col md:flex-row bg-gray-50">
      ...
      <aside className="hidden md:flex w-64 flex-col bg-white border-r min-h-[100dvh]">
        <div className="p-4 border-b flex items-center gap-2">
          ...
          <span className="font-bold text-xl text-primary">FitAdmin</span>
        </div>
      ...
      <header className="md:hidden bg-white border-b h-14 flex items-center justify-between px-4 sticky top-0 z-30">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-lg text-primary">FitAdmin</span>
        </div>
        <Sheet>
